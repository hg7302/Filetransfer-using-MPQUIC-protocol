...
type session struct {
	...
	openPaths []protocol.PathID         //  needed for round-robin path sending, will not delete record when a path is closed
...
//  add localPconn net.PacketConn for convenience of path creation
func (s *session) handleFramesNew(fs []wire.Frame, p *path, localPconn net.PacketConn) error {
	for _, ff := range fs {
		...
		case *wire.PathsFrame:
			...
			//   server check if there are new paths to create
			if s.perspective == protocol.PerspectiveServer {
...
//  check if we have RTT statistics of all paths
...
//  check if we have Bandwidth statistics of all paths
...
//  send ping for all paths on a low-rtt path
...
func (s *session) garbageCollectStreams() {
	s.streamsMap.Iterate(func(str *stream) (bool, error) {
		...
		if str.finished() {
			...
			//  delete records about this stream assigned to path
