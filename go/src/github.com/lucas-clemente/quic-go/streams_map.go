...
func (m *streamsMap) GetOrOpenStream(id protocol.StreamID) (*stream, error) {
	...
	for ; sid <= id; sid += 2 {
		...
		if utils.Debug() {
			utils.Debugf("  Get or open stream %d on perspective %x\n", sid, m.perspective)
		}
	}
	...
	//fmt.Printf("StreamID= %d",m.streams[id])
...
func (m *streamsMap) GetOrOpenStreamPriority(id protocol.StreamID, priority *protocol.Priority) (*stream, error) {
	...
	for ; sid <= id; sid += 2 {
		...
		if utils.Debug() {
			utils.Debugf("  GetOrOpenStreamPriority: Get or open stream %d with priority %d on perspective %x\n", sid, priority, m.perspective)
		}
	}
...
func (m *streamsMap) GetOrOpenStreamPrioritySize(id protocol.StreamID, priority *protocol.Priority) (*stream, error) {
	...
	for ; sid <= id; sid += 2 {
		...
		if utils.Debug() {
			utils.Debugf("  GetOrOpenStreamPrioritySize: Get or open stream %d with priority %d, size %d on perspective %x\n", sid, priority.Weight, m.perspective)
		}
	}
...
func (m *streamsMap) openStreamPriorityImpl(priority *protocol.Priority) (*stream, error) {
	...
	if utils.Debug() {
		utils.Debugf("  openStreamPriorityImpl(called by OpenStreamPrioritySync): stream %d, priority %d, perspective %x\n", s.streamID, s.priority, m.perspective)
	}
	return s, nil
}
...
func (m *streamsMap) openStreamPrioritySizeImpl(priority *protocol.Priority) (*stream, error) {
	...
	if utils.Debug() {
		utils.Debugf("  openStreamPrioritySizeImpl(called by OpenStreamPrioritySizeSync): stream %d, priority %d, size%d, perspective %x\n", s.streamID, s.priority.Weight, m.perspective)
	}
...
//  perform sending data of stream 1 and 3, until there is no data or window
...
//   pop streamframe of streams reside in this path round-robinly
...
//   pop streamframe of streams reside in this path (proportinally according to priority)
func (m *streamsMap) PriorityIteratePopOfPath(fn streamLambda, pth *path) error {
	//    calculate pop probability of each stream on this path based on priority
	m.mutex.Lock()
	defer m.mutex.Unlock()
	...
	for i := 0; i < len(pth.streamIDs); i++ {
		...
		// }
		//   we prioritize stream 3 if either of them in this path, crypto stream (stream 1) is handled separately
		if sid == 3 {
			cont, err := m.iterateFunc(sid, fn)
			// if utils.Debug() {
	...
	for i := 0; i < len(pth.streamIDs); i++ {
		...
		// if utils.Debug() {
		// 	utils.Debugf("  The probability for stream %d on path %d to pop is %f", sid, pth.pathID, probability[sid])
		// }
...
//   implement sort interface to sort priority of streams
